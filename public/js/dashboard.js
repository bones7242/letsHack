...
$(document).ready(function(){
    ...
    $(".challenge-link").on("click", function(){
        ...
        var challengeEditForm = $("#edit-challenge-field");
        // clear in inner html
        challengeEditForm.empty();
        ...
        challengeEditForm.append("<div><textarea name='instructionsAll' rows='4' cols='100'>" + $(this).attr("data-instructionsAll") + "</textarea></div>");
        // instructions to A
        challengeEditForm.append("<div><label for='instructionsA'>Instructions for User A</label></div>");
        challengeEditForm.append("<div><textarea name='instructionsA' rows='4'>" + $(this).attr("data-instructionsA") + "</textarea></div>");
        // instructions to B
        challengeEditForm.append("<div><label for='instructionsB'>Instructions for User B</label></div>");
        challengeEditForm.append("<div><textarea name='instructionsB' rows='4'>" + $(this).attr("data-instructionsB") + "</textarea></div>");
        // start code A
        challengeEditForm.append("<div><label for='startCodeA'>Start Code for User A</label></div>");
        challengeEditForm.append("<div><textarea name='startCodeA' rows='4'>" + $(this).attr("data-startCodeA") + "</textarea></div>");
        // start code B
        challengeEditForm.append("<div><label for='startCodeB'>Start Code for User B</label></div>");
        challengeEditForm.append("<div><textarea name='startCodeB' rows='4'>" + $(this).attr("data-startCodeB") + "</textarea></div>");
        // test A 
        challengeEditForm.append("<div><label for='testA'>Test for user A</label></div>");
        challengeEditForm.append("<div><textarea name='testA' rows='4'>" + $(this).attr("data-testA") + "</textarea></div>");
        // test B 
        challengeEditForm.append("<div><label for='testB'>Test for user B</label></div>");
        challengeEditForm.append("<div><textarea name='testB' rows='4'>" + $(this).attr("data-testB") + "</textarea></div>");
        // submit button
        challengeEditForm.append("<div><button type='submit'>Submit</button></div>");
    })
